...
export default function Glossary() {
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      terms = terms.filter(t => t.term.toLowerCase().includes(s) || t.definition.toLowerCase().includes(s));
    }
    if (activeLetter) {
    ...
    <div className="page-container max-w-3xl">
      <div className="text-center mb-8">
        ...
        <p className="mt-2 text-base" style={{ color: 'var(--color-text-light)' }}>
          A-Z glossary of U.S. immigration terms and definitions.
        </p>
      </div>

      <SearchBar placeholder="Search terms..." onSearch={setSearch} className="max-w-lg mx-auto mb-6" />
                ...
                <div className="px-4 pb-4 border-t animate-fade-in" style={{ borderColor: 'var(--color-border)' }}>
                  ...
                  <div className="rounded-lg p-3 mb-3" style={{ backgroundColor: 'var(--color-surface)' }}>
                    <p className="text-xs font-medium italic" style={{ color: 'var(--color-text-light)' }}>
                      <strong>Example:</strong> {term.context}
                    </p>
                  </div>
                  {term.relatedTerms.length > 0 && (
                    <div className="flex flex-wrap items-center gap-1">
                      <span className="text-xs font-medium" style={{ color: 'var(--color-text-light)' }}>Related:</span>
                  ...
                  >
                    <MessageSquare className="h-3 w-3" /> Explain Simply
                  </Link>
                </div>
              )}
            </div>
      ...
      {filtered.length === 0 && (
        <div className="text-center py-12">
          <p className="text-lg" style={{ color: 'var(--color-text-light)' }}>No terms match your search.</p>
        </div>
      )}
    </div>
  );
